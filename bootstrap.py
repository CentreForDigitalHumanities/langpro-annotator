...
def activate_frontend():
    ...
    if framework == 'backbone':
        ...
        project_name = 'langpro_annotator'.replace('_', '-')
        Command(
        ...
        Command(
            'Creating project',
            ['yarn', 'ng', 'new', project_name, '--prefix=la',
                '--ssr',
                '--skip-git=true',
                '--skip-install=true',
        ...
        if '4200' != '4200':
            Command(
                'Set frontend port',
                ['yarn', 'ng', 'config', "projects.langpro-annotator.architect.serve.options.port", '4200'],
                cwd="frontend"
            )()
    else:
